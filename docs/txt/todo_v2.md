...
- [x] 23.1 리포트 생성 시 70C 차감 (`POST /api/profiles/[id]/report`)
- [x] 23.2 섹션 재분석 시 5C 차감 (`POST /api/profiles/[id]/report/reanalyze`)
- [x] 23.3 크레딧 부족 시 안내 (`InsufficientCreditsDialog`)

**구현 내용**:
- `SERVICE_CREDITS.profileReport = 70`, `sectionReanalysis = 5` 추가
...
| 2.0.7 | 2026-01-03 | Task 22~23 완료 - 로딩 UI (폴링 기반 진행률), 크레딧 연동 (70C 리포트, 5C 재분석), 5개 언어 번역 |
