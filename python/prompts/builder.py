...
```json
{
  ...
  "summary": "癸水(계수)는 이슬비처럼 조용하고 섬세한 물 기운입니다. 큰 바다(임수)와 달리 눈에 띄지 않게 스며드는 지혜와 관찰력을 가지고 있습니다... (500자 이상 상세 작성)"
}
```''',
    'personality': '''
```json
...
**type**: 격국명을 반드시 한글로만 작성! (예: 식신격, 상관격, 정관격). ⚠️ 한자 사용 금지!
**typeChinese**: 격국 한자 (예: 食神格, 傷官格)
...
**필수 포함 내용**:
1. **일간 설명**: 이 사람의 일간이 무엇이고 어떤 특성인지 일상적 비유로 설명
   - 예: "乙木(을목)은 풀이나 덩굴처럼 유연하고 적응력이 뛰어난 기운입니다.
     큰 나무(갑목)처럼 우직하게 버티기보다는, 바람이 불면 휘어지며 살아남는
     지혜로운 생존력을 가집니다."
2. **사주 구성 해석**: 월지, 시주 등 전체 구성이 일간에 어떤 영향을 미치는지
3. **격국의 의미**: 왜 이 격국이 형성되었고, 삶에 어떤 영향을 주는지
4. **용신/기신 설명**: 왜 이 오행이 필요하고, 왜 저 오행은 피해야 하는지
   쉬운 비유로 설명
5. **전체적인 인생 흐름**: 이 사주의 주인이 어떤 삶의 방향성을 가지는지

**작성 톤**: 전문 용어를 쓰되, 반드시 괄호 안에 쉬운 설명을 덧붙이세요.

### 한자 용어 설명 규칙 (필수 - 반드시 준수!)
summary에서 한자가 등장할 때 **예외 없이** 한글 음독을 괄호 안에 함께 표기하세요.
...
**예시**:
- "乙木(을목) 일간은 풀이나 덩굴처럼 유연합니다..."
- "食神格(식신격)은 재능을 발휘하여 재물을 만드는 격입니다..."
- "申月(신월, 가을 금속 기운)에 태어나..."
...
**Must include**:
1. **Day Master explanation**: What the Day Master is and its characteristics using everyday metaphors
   - Example: "Yi Wood (乙木) is like grass or vines - flexible and adaptable.
     Unlike the sturdy Jia Wood (大树), it survives by bending with the wind,
     showing wisdom in resilience."
2. **Chart interpretation**: How the Month Branch, Hour Pillar affect the Day Master
3. **Structure meaning**: Why this structure formed and how it affects life
4. **Useful/Harmful God explanation**: Why certain elements are needed and others avoided,
   using easy metaphors
5. **Overall life direction**: What life path this chart suggests

**Writing tone**: Use professional terms but always add easy explanations in parentheses.
...
### 4. 四柱要約（summary）の作成規則 [重要！]
**500文字以上**で詳しく作成し、初心者でも理解できるように説明してください。

**必須内容**：
1. **日干の説明**: この人の日干が何で、どのような特性かを日常的な比喩で説明
   - 例: 「乙木は草や蔓のように柔軟で適応力に優れた気です。
     大木（甲木）のように頑固に耐えるのではなく、風が吹けば曲がりながら生き残る
     賢い生存力を持っています。」
2. **四柱構成の解釈**: 月支、時柱などの全体構成が日干にどう影響するか
3. **格局の意味**: なぜこの格局が形成され、人生にどう影響するか
4. **用神/忌神の説明**: なぜこの五行が必要で、あの五行は避けるべきか簡単な比喩で説明
5. **全体的な人生の流れ**: この四柱の持ち主がどのような人生の方向性を持つか

### 漢字用語説明規則（必須）
漢字用語が出る時は必ず**日常的な比喩 + 読み方**と共に説明してください。

**天干の比喩**: 甲-大木, 乙-草/蔓, 丙-太陽, 丁-蝋燭, 戊-山, 己-田畑, 庚-岩, 辛-宝石, 壬-海, 癸-露
...
### 4. 四柱总结（summary）撰写规则 [重要！]
请**500字以上**详细撰写，让初学者也能理解。

**必须包含的内容**：
1. **日干说明**: 用日常比喻解释这个人的日干是什么，有什么特性
   - 例: "乙木像草藤一样柔软有适应力。与大树（甲木）不同，
     它不是顽强地硬撑，而是随风弯曲生存，拥有智慧的生存力。"
2. **四柱构成解读**: 月支、时柱等整体构成如何影响日干
3. **格局的意义**: 为什么形成这个格局，对人生有什么影响
4. **用神/忌神说明**: 用简单比喻解释为什么需要这个五行，为什么要避免那个五行
5. **整体人生方向**: 这个四柱的主人有什么样的人生方向

### 汉字术语说明规则（必须）
出现汉字术语时，务必配合**日常比喻**进行解释。

**天干比喻**: 甲-大树, 乙-草/藤, 丙-太阳, 丁-蜡烛, 戊-山, 己-田地, 庚-岩石, 辛-宝石, 壬-大海, 癸-露水
...
### 4. 四柱總結（summary）撰寫規則 [重要！]
請**500字以上**詳細撰寫，讓初學者也能理解。

**必須包含的內容**：
1. **日干說明**: 用日常比喻解釋這個人的日干是什麼，有什麼特性
   - 例: 「乙木像草藤一樣柔軟有適應力。與大樹（甲木）不同，
     它不是頑強地硬撐，而是隨風彎曲生存，擁有智慧的生存力。」
2. **四柱構成解讀**: 月支、時柱等整體構成如何影響日干
3. **格局的意義**: 為什麼形成這個格局，對人生有什麼影響
4. **用神/忌神說明**: 用簡單比喻解釋為什麼需要這個五行，為什麼要避免那個五行
5. **整體人生方向**: 這個四柱的主人有什麼樣的人生方向

### 漢字術語說明規則（必須）
出現漢字術語時，務必配合**日常比喻**進行解釋。

**天干比喻**: 甲-大樹, 乙-草/藤, 丙-太陽, 丁-蠟燭, 戊-山, 己-田地, 庚-岩石, 辛-寶石, 壬-大海, 癸-露水
